        ...
        <el-card v-if="show && checkPermission(['admin','role_all','role_edit'])" class="box-card">
          <div slot="header" class="clearfix">
            ...
            <el-button
              ...
              type="success"
              style="float: right; padding: 4px 8px"
              @click="is_permissions=true; toSave()"
            >保存</el-button>
          </div>
          <el-tree
            ref="permission"
...
export default {
  ...
  methods: {
    ...
    menuCheckChange(data, checked) {
      if (checked && data.pid) {
        this.$refs.menu.setChecked(data.pid, true);
      } else if (!checked && data.children && data.children.length > 0) {
        const node = this.$refs.menu.getNode(data.id);
        if (node.childNodes.some(item => item.checked)) {
          this.$refs.menu.setChecked(data.id, true);
        }
      }
    },
